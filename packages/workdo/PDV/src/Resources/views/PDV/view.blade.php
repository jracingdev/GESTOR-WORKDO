...
{{__('PDV Order')}},
{{\workdo\PDV\Entities\PDV::pdvNumberFormat($pdv->pdv_id) }}
@endsection
@section('page-action')
    @permission('pdv order manage')
        <div class="float-end">
            <a href="{{ route('pdv.pdf', Crypt::encrypt($pdv->id))}}" class="btn btn-sm btn-primary" target="_blank" data-bs-toggle="tooltip" data-title="{{ __('Download') }}" title="{{ __('Download') }}"><i class="ti ti-download"></i></a>
        </div>
    @endpermission
@endsection
@section('content')
                ...
                <div class="invoice-print">
                            ...
                            <div class="d-flex invoice-wrp flex-wrap align-items-center gap-md-2 gap-1">
                                <div class="d-flex invoice-date flex-wrap align-items-center gap-md-3 gap-1">
                                    <p class="mb-0"> <strong>{{__('Issue Date')}} :</strong>
                                        {{company_date_formate($pdv->pdv_date)}}</p>
                                </div>
                    ...
                    <div class="p-sm-4 p-3 invoice-billed">
                        <div class="row row-gap">
                            ...
                            <div class="col-lg-4 col-sm-6">
                                @if(isset($company_setting['pdv_shipping_display']) && $company_setting['pdv_shipping_display']=='on')
                        ...
                        <div class="table-responsive mt-2">
                            <table class="table mb-0 table-striped">
                                ...
                                @foreach($iteams as $key =>$iteam)
                                    @if(!empty($iteam->tax))
                                    @php
                                                    $taxes=\workdo\PDV\Entities\PDV::tax($iteam->tax);
                                                    $totalQuantity+=$iteam->quantity;
                                                    $totalRate+=$iteam->price;
                                                    $totalDiscount+=$iteam->discount;
                                                    foreach($taxes as $taxe){
                                                        $taxDataPrice=\workdo\PDV\Entities\PDV::taxRate($taxe->rate,$iteam->price,$iteam->quantity);
                                            ...
                                            @if(!empty($iteam->tax))
                                                <table class="w-100">
                                                    ...
                                                    @foreach($taxes as $tax)
                                                        @php
                                                            $taxPrice=\workdo\PDV\Entities\PDV::taxRate($tax->rate,$iteam->price,$iteam->quantity);
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <td>{{currency_format_with_sym($pdvPayment['amount'])}}</td>
                                    </tr>
                                    <tr>
                                        <td colspan="5"></td>
                                        <td><b>{{__('Discount')}}</b></td>
                                        <td>{{currency_format_with_sym($pdvPayment['discount'])}}</td>
                                    </tr>
                                    <tr>
                                        <td colspan="5"></td>
                                        <td><b>{{__('Total')}}</b></td>
                                        <td>{{currency_format_with_sym($pdvPayment['discount_amount'])}}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
