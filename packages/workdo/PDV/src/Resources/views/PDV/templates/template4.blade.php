...
<head>
    ...
    <title>{{ \workdo\Pos\Entities\Pos::pdvNumberFormat($pdv->pdv_id)}} | {{ !empty(company_setting('title_text')) ? company_setting('title_text') : (!empty(admin_setting('title_text')) ? admin_setting('title_text') :'WorkDo') }}</title>
    <link
    ...
    <div class="invoice-preview-main" id="boxes">
                            ...
                            <div style="justify-content: end; display: flex;">
                                ...
                                <table class="no-space">
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{ __('Number: ')}}</td>
                                            <td class="text-right">{{\workdo\Pos\Entities\Pos::pdvNumberFormat($pdv->pdv_id)}}</td>
                                        </tr>
                                        <tr>
                                            <td>{{ __('POS Date:')}}</td>
                                            <td class="text-right">{{company_date_formate($pdv->pdv_date)}}</td>
                                        </tr>
                                    </tbody>
                                </table>
                        </td>
        ...
        <div class="invoice-body">
            <table>
                <tbody>
                    <tr>
                        ...
                        @if($settings['pdv_shipping_display']=='on')
                            <td class="text-right">
            ...
            <table class="add-border invoice-summary" style="margin-top: 30px;">
                ...
                <tbody>
                    @if(isset($pdv->itemData) && count($pdv->itemData) > 0)
                    @foreach($pdv->itemData as $key => $item)
                    <tr>
                        <td>{{$item->name}}</td>
                        <td>{{$item->quantity}}</td>
                ...
                <tfoot>
                    <tr>
                        <td>{{__('Total')}}</td>
                        <td>{{$pdv->totalQuantity}}</td>
                        <td>{{currency_format_with_sym($pdv->totalRate)}}</td>
                        <td>-</td>
                        <td>{{currency_format_with_sym($pdv->totalTaxPrice) }}</td>
                        ...
                        <td colspan="2" class="sub-total">
                            <table class="total-table">
                                ...
                                <tr>
                                    <td>{{__('Total')}}:</td>
                                    @if(!empty($pdvPayment->discount))
                                        <td>{{currency_format_with_sym($pdvPayment->amount - $pdvPayment->discount)}}</td>
                                    @else
                                       <td>{{currency_format_with_sym($pdvPayment->amount)}}</td>
                                    @endif
                                </tr>
            ...
            <div class="invoice-footer">
                <P>{{$settings['pdv_footer_notes']}}</P>
                <h6>{{$settings['pdv_footer_title']}}</h6>
            </div>
        </div>
    </div>
    @if(!isset($preview))
         @include('pdv::pdv.script');
    @endif
</body>
</html>
