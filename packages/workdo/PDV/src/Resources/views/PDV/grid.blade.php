...
@section('page-action')
    <div class="d-flex">
        @stack('addButtonHook')
        <a href="{{ route('pdv.report') }}" class="btn btn-sm btn-primary btn-icon" data-bs-toggle="tooltip"
        title="{{ __('List View') }}">
...
<div class="row row-gap-2 mb-4">
    @if(count($pdvPayments) > 0)
        @foreach ($pdvPayments as $pdvPayment)
                    ...
                    <div class="card-header border border-bottom p-3 h-100">
                        <div class="user-img-wrp d-flex align-items-center">
                            <div class="user-image rounded border-2 border border-primary">
                                <img
                                    @if(!empty($pdvPayment->avatar))
                                src="{{(!empty($pdvPayment->avatar))? get_file("profile/".$pdvPayment->avatar): asset(url("./assets/img/clients/160x160/img-1.png"))}}"
                                @else
                                @if($pdvPayment->customer_id == 0)
                                avatar="{{__('Walk-in Customer')}}"
                                @else
                                avatar="@if(module_is_active('Account')) {{!empty($pdvPayment->customer) ? $pdvPayment->customer->name : $pdvPayment->user->name}} @else ''
                                @endif "
                                @endif
                                @endif
                            ...
                            <div class="user-content">      
                                @if($pdvPayment->customer_id == 0)
                                    <a href="{{  route('pdv.show',\Crypt::encrypt($pdvPayment->id)) }}" data-title="{{__('Purchase Details')}}" class="action-item text-primary mt-2">
                                        <h4 class="mb-2">{{ ucfirst('Walk-in Customer') }}</h4>
                                    </a>
                                @else
                                    @if(module_is_active('Account'))
                                        <a href="{{  route('pdv.show',\Crypt::encrypt($pdvPayment->id)) }}" data-title="{{__('Purchase Details')}}" class="action-item text-primary mt-2">
                                            <h4 class="mb-2">{{ ucfirst(!empty($pdvPayment->customer) ? $pdvPayment->customer->name :$pdvPayment->user->name) }}</h4>
                                        </a>
                                    @else
                                        <a href="{{  route('pdv.show',\Crypt::encrypt($pdvPayment->id)) }}" data-title="{{__('Purchase Details')}}" class="action-item text-primary mt-2">
                                            <h4 class="mb-2">{{ ucfirst(!empty($pdvPayment->user) ? $pdvPayment->user->name : '-') }}</h4>
                                        </a>
                                    @endif
                                @endif
                                <span class="text-dark text-md">{{ !empty($pdvPayment->warehouse_id) ? $pdvPayment->warehouse->name : '-' }}</span>
                            </div>
                        </div>
                    </div>
                    <div class="card-body text-center p-3">
                        <div class="bottom-icons d-flex flex-wrap align-items-center justify-content-between">
                            <div class="edit-btn-wrp d-flex flex-wrap align-items-center">
                                @permission('pdv show')
                                <a href="{{ route('pdv.show',\Crypt::encrypt($pdvPayment->id)) }}"
                            ...
                            @permission('pdv show')
                                <a class="badge p-2 px-3 bg-primary" href="{{ route('pdv.show',\Crypt::encrypt($pdvPayment->id)) }}">
                                    {{ \workdo\PDV\Entities\PDV::pdvNumberFormat($pdvPayment->pdv_id) }}
                                </a>
                            @else
        ...
        @endforeach
        {!! $pdvPayments->links('vendor.pagination.global-pagination') !!}
    @else
