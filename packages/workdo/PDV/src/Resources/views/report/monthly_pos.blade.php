...
@push('scripts')
    ...
    <script>
        (function () {
            ...
            var arChart = new ApexCharts(document.querySelector("#monthly-pdv"), chartBarOptions);
            arChart.render();
        })();
    </script>
@endpush
...
@section('content')
    ...
    <ul class="nav nav-pills my-3" id="pills-tab" role="tablist">
        <li class="nav-item">
            <a class="nav-link" id="pills-home-tab" data-bs-toggle="pill"
               href="{{ route('report.daily.pdv') }}"
               onclick="window.location.href = '{{ route('report.daily.pdv') }}'" role="tab"
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="mt-2 " >
                <div class="card">
                    <div class="card-body">
                        {{ Form::open(['route' => ['report.monthly.pdv'], 'method' => 'GET', 'id' => 'monthly_pdv_report_submit']) }}
                        <div class="row d-flex align-items-center justify-content-end">
                            ...
                            <div class="col-auto float-end ms-2 mt-4 d-flex">
                                <a href="#" class="btn btn-sm btn-primary me-2"
                                   onclick="document.getElementById('monthly_pdv_report_submit').submit(); return false;"
                                   data-toggle="tooltip" data-original-title="{{ __('apply') }}" title="{{ __('Apply') }}">
                                    <span class="btn-inner--icon"><i class="ti ti-search"></i></span>
                                </a>
                                <a href="{{ route('report.monthly.pdv') }}" class="btn btn-sm btn-danger" data-toggle="tooltip"
    ...
    <div id="printableArea">
        <div class="row mt-0">
            <div class="col">
                <input type="hidden" value="{{$filter['warehouse'].' '.__('Monthly PDV').' '.'Report of'.' '.$filter['startMonth'].' to '.$filter['endMonth']}}" id="filename">
                <div class="card p-4 mb-4">
                    <h7 class="report-text gray-text mb-0">{{__('Report')}} :</h7>
                    <h6 class="report-text mb-0">{{__('Monthly PDV Report')}}</h6>
                </div>
            </div>
