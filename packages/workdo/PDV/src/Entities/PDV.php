...
class PDV extends Model
{
    ...
    public function posPayment(){
        return $this->hasOne(PDVPayment::class,'pos_id','id');
    }

    public function PDVProduct(){

        return $this->hasMany(PDVProduct::class,'pos_id','pos_id');
    }

    public function items()
    {
        return $this->hasMany(PDVProduct::class, 'pos_id', 'id');
    }
    public function itemswithproduct()
    {
        return $this->hasMany(PDVProduct::class, 'pos_id', 'id')->with('product');
    }
    ...
    {
        $sells = PDV::where('created_by', creatorId())
    ...
    {
        $subtotals = 0;
        foreach($this->items as $item)  //PDVProduct
        {
    ...
    {
        $sales = PDV::whereDate('created_at', '>', Carbon::now()->subDays(10))->where('created_by', creatorId())->where('workspace',getActiveWorkSpace())->orderBy('created_at')->get()->groupBy(
                function ($val){
